...
def get_courses(tsv):
    ...
    with open(tsv, newline='') as tsv_file:
        ...
        next(courses)
        for row in courses:
            number, url, section, season, year = row
            courses_list.append({
                'number': number,
                'url': url,
                'section': section,
...
def main():
    ...
    for course in get_courses(courses_tsv):
        ...
        course_number = course_names.get(course.get('number')).get('number')
        course_number_uppercase = course_number.upper()
        ...
        body.append('<li>' + format_link(
            url=course.get('url'),
            target=f'{course_name} <span class="course_number">{course_number_and_maybe_section}</span>'
        ) + '</li>')
    ...
    # output the data.
    print(
        get_header(title=page_title, stylesheet=css_stylesheet),
        "\n".join(body),
        get_footer()
    )
